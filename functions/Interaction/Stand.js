...
if (!chance) { // bot passes
  ...
  if (totalPlayerCardValue === defaultBotValue) { // If Draw
    await Draw({ bet, totalPlayerCardValue, totalBotCardValue: defaultBotValue, yourHand, resultDealerHand, userPfp }, event);
    return;
...
if (totalPlayerCardValue === totalBotCardValue) { // If Draw
  await Draw({ bet, totalPlayerCardValue, totalBotCardValue, yourHand, resultDealerHand, userPfp }, event);
  return;
} else if (totalBotCardValue > 21) { // if bot's card exceed to 21
  await Win({ bet, totalPlayerCardValue, totalBotCardValue, yourHand, resultDealerHand, userPfp }, event);
  return;
} else if (totalPlayerCardValue < totalBotCardValue) { // If Player Lose
  await Lose({ bet, totalPlayerCardValue, totalBotCardValue, yourHand, resultDealerHand, userPfp }, event);
  return;
